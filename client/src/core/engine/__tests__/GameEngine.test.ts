...
import { describe, beforeEach, test, expect, it } from ".pnpm/vitest@4.1.10_@types+node@26.1.2_vite@8.2.0_@types+node@26.1.2_terser@5.49.0_yaml@2.9.0_/node_modules/vitest/dist";
...
describe("GameEngine Core Mechanics", () => {
  ...
  test("locks piece upon hard drop and updates grid", () => {
    ...
    const hasGemsOnBoard = state.grid.some((row) =>
      row.some((gem) => gem !== null),
    );
