...
import { GRID_WIDTH, CELL_SIZE } from "../constants";
import BasePlayer from "../players/BasePlayer";

export default class BaseMatchScene extends Phaser.Scene {
  private readonly player1: BasePlayer; // local player
  private readonly player2: BasePlayer; // bot or online player
  private gameWidth = 0;
  private gameHeight = 0;
  private readonly gameAreaWidth = GRID_WIDTH * CELL_SIZE;

  constructor(player1: BasePlayer, player2: BasePlayer) {
    super("BaseMatchScene");
  ...
  handleWin() {
    ...
    const score = this.player1.game.getScore() ?? 0;
