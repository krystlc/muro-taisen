...
describe('Advanced Puzzle Mechanics: Power Gems, Monolithic Gravity, & Counters', () => {
  ...
  it('triggers a Power Gem explosion chain when a matching Crash Gem hits it', () => {
    ...
    for (let r = 0; r < BOARD_ROWS; r++) for (let c = 0; c < BOARD_COLS; c++) grid[r][c] = null;

    // Create a 2x2 Blue Power Gem at bottom-left (rows 0-1, cols 0-1)
    const powerId = 'power_0_0_test';
    grid[0][0] = { id: 'p00', color: GemColor.BLUE, type: GemType.NORMAL, powerGemId: powerId, powerWidth: 2, powerHeight: 2 };
    grid[0][1] = { id: 'p01', color: GemColor.BLUE, type: GemType.NORMAL, powerGemId: powerId };
    grid[1][0] = { id: 'p10', color: GemColor.BLUE, type: GemType.NORMAL, powerGemId: powerId };
    grid[1][1] = { id: 'p11', color: GemColor.BLUE, type: GemType.NORMAL, powerGemId: powerId };
  ...
  it('forces monolithic Power Gems to require a fully unobstructed column floor span to fall', () => {
    ...
    grid[2][0] = { id: 'p20', color: GemColor.RED, type: GemType.NORMAL, powerGemId: powerId, powerWidth: 2, powerHeight: 2 };
    grid[2][1] = { id: 'p21', color: GemColor.RED, type: GemType.NORMAL, powerGemId: powerId };
    grid[3][0] = { id: 'p30', color: GemColor.RED, type: GemType.NORMAL, powerGemId: powerId };
    grid[3][1] = { id: 'p31', color: GemColor.RED, type: GemType.NORMAL, powerGemId: powerId };

    // Place an obstacle underneath column 1 at row 0, leaving column 0 completely empty underneath
    grid[0][1] = { id: 'obstacle', color: GemColor.GREEN, type: GemType.NORMAL };
  ...
  it('decrements Counter Gems and tracks counter values correctly over turns', () => {
    ...
    grid[0][0] = { id: 'counter1', color: GemColor.RED, type: GemType.COUNTER, counterValue: 2 };
